...
class App(QMainWindow):
    ...
    def _set_theme(self, theme: str = "dev"):
        ...
        try:
            with open(f"{Assets.THEMES.value}{theme}.scss", "r") as file:
                self.setStyleSheet(
                    sass.compile(
                        string=file.read(),  # read the content of the file
                        include_paths=[
                            Assets.THEMES.value,
    ...
    def assign_actions(self):
        ...
        qactions = list(
            filter(lambda x: x.text() != "", self.menu_bar.findChildren(QAction))
        )
        # [print(action.text()) for action in qactions]  # print the text of the actions

        # * Add the behavior to the actions
        # open a new file
        qactions[1].triggered.connect(
            lambda: self.display.workspace.v_list[
                self.display.workspace.currentIndex()
            ].open_file()
        )
